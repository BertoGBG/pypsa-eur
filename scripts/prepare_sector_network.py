...
def define_spatial(nodes, options):
    ...
    spatial.clinker.location = nodes
...
def add_rock_weathering(n, costs):
    ...
    rock_weathering_land = pd.read_csv(
...
# trajectory) and add_energy_limit_per_carrier (independent per-carrier
...
def add_fossil_fuel_limit(n, costs, config, investment_year):
    ...
    add_energy_limit_per_carrier — the two are structurally separate
...
def add_energy_limit_per_carrier(n, costs, config, investment_year):
    """
    ...
    vs `energy_limit_per_carrier`) and can be active separately or together;
    ...
    costs : pd.DataFrame
    config : dict
        Reads `energy_limit_per_carrier` (bool) and
        `energy_limit_per_carrier_values` ({carrier: {year: MtCO2}}), analogous
        in structure to `fossil_limit`/`fossil_limit_values`.
    investment_year : int
        Planning horizon year (e.g. 2030, 2035, …).
    """
    if not config.get("energy_limit_per_carrier", False):
        return

    limits = config.get("energy_limit_per_carrier_values", {})
    if not limits:
        logger.warning(
            "energy_limit_per_carrier enabled but energy_limit_per_carrier_values "
            "is empty. Skipping constraint."
        )
        return

    intensities = _resolve_fossil_co2_intensities(
        n, costs, label="energy_limit_per_carrier | "
    )
    ...
    for group, members in CARRIER_GROUPS.items():
        ...
        if F_t is None:
            logger.warning(
                f"energy_limit_per_carrier | '{group}': no value found "
                f"in energy_limit_per_carrier_values for year={investment_year}. "
        ...
        if not present:
            logger.info(
                f"energy_limit_per_carrier | '{group}': no matching "
        ...
        for carrier in present:
            ...
            if val is None:
                logger.warning(
                    f"energy_limit_per_carrier | '{group}': carrier "
        ...
        if not any_set:
            logger.warning(
                f"energy_limit_per_carrier | '{group}': no carrier had a "
                f"resolved CO₂ intensity — skipping this cap entirely."
            )
            continue

        logger.info(
            f"Adding per-carrier energy-security limit: '{group}' <= {F_t:.2f} "
            f"MtCO₂-eq for {investment_year} (independent of fossil_fuel_limit; "
            f"CCS provides no credit)."
        )
        n.add(
            "GlobalConstraint",
            f"energy_limit_security_{group}",
            type="primary_energy",
...
def add_biomass(
    ...
    if options["solid_biomass_import"].get("enable", False):
        ...
        # max_amount [TWh/yr]: either a flat number (upstream default) or a
        # {year: TWh} dict for myopic year-indexing (this fork's addition --
        # see config comment for the self-sufficiency-fraction sourcing).
        # Resolved via get() the same way as e.g. fossil_limit_values.
        biomass_import_max_amount_twh = get(
            options["solid_biomass_import"]["max_amount"], investment_year
        )
        # convert TWh in MWh
        biomass_import_max_amount = biomass_import_max_amount_twh * 1e6 * nyears
...
def add_low_t_industry(n, nodes, industrial_demand, costs, must_run):
    ...
    Adds a lowT industry bus + load at each node, then optionally adds
    technology links (solid biomass boiler, gas boiler, heat pump, electric
    boiler, methanol combustion) based on config options["industry_t"]["low_T"].
    The load is taken from the "solid biomass" column of industrial_demand,
    which represents the existing low-T heat demand previously met by biomass.
    """
    logger.info("Add low temperature industry heat (endogenous).")
...
def add_medium_t_industry(n, nodes, industrial_demand, costs, must_run):
    ...
    Demand is share_medium × industry methane demand, representing the
    fraction of gas heat in the 200–500 °C range.
    """
    logger.info("Add medium temperature industry heat (endogenous).")
    ...
    if options["industry_t"]["medium_T"]["biomass"]:
        marginal_cost = (
            costs.at["direct firing solid fuels", "VOM"]
    ...
    if options["industry_t"]["medium_T"]["hydrogen"]:
        # TODO: research cost of industrial H2 combustion; currently set to 10x gas
        n.add(
            "Link",
            nodes,
...
def add_high_t_industry(n, nodes, industrial_demand, costs, must_run):
    ...
    Demand is share_high × industry methane demand, representing the
    fraction of gas heat above ~500 °C.
    """
    logger.info("Add high temperature industry heat (endogenous).")
...
def add_exogen_t_industry(n, nodes, industrial_demand, costs, nhours):
    """
    Add industry heat with exogenous (fixed) supply technology mix.

    This reproduces the original behaviour when endogen=false:
      - low-T heat is supplied by solid biomass (existing bus structure)
      - medium/high-T heat is supplied by gas
    """
...
def adjust_industry_demand(nodes):
    ...
    endogenous_sector = snakemake.params.sector["endogenous_sectors"]["subsectors"]
...
def add_industry(
    ...
    if snakemake.params.sector["endogenous_sectors"]["enable"]:
    ...
    # Dispatch heat supply for industry: endogenous (optimised technology mix
    # per temperature band) or exogenous (fixed biomass/gas assignment).
    ...
    if (
        snakemake.params.sector["endogenous_sectors"]["enable"]
        and "steel" in snakemake.params.sector["endogenous_sectors"]["subsectors"]
    ):
        # add steel load
        ...
        # structure allows switching from gas to H2
        n.add("Carrier", "DRI reduction agent")
        n.add(
            "Bus",
            spatial.nodes,
        ...
        # gas reduction agent
        # bus to decide whether to capture emissions or not
        n.add("Carrier", "steel emission")
        n.add(
            "Bus",
            spatial.nodes,
        ...
        n.add(
            ...
            bus2=spatial.nodes + " DRI reduction",
    ...
    if (
        snakemake.params.sector["endogenous_sectors"]["enable"]
        and "cement" in snakemake.params.sector["endogenous_sectors"]["subsectors"]
    ):
        ...
        n.add(
            ...
            location=spatial.clinker.location,
            carrier="clinker",
            unit="t",
        )
...
def add_shipping(
    ...
    """
    Add shipping (navigation) fuel demand to the network, either
    endogenously (competing fuel Links on a shared demand bus, the
    optimiser picks the mix) or exogenously (fixed year-indexed fuel
    shares), selected by ``options["shipping_endogenous"]``. See
    ``_add_shipping_endogenous`` / ``_add_shipping_exogenous`` for details
    of each.
    """
...
if __name__ == "__main__":
    ...
    add_energy_limit_per_carrier(n, costs, snakemake.config, investment_year)
