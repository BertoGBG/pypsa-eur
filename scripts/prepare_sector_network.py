...
def add_afforestation(n, costs):
    ...
    # the uniform rate (density). Call redistribute_afforestation_seasonal() from
    ...
    if potential_type == "density":
        ...
        # Node-specific capital cost: I * (annuity(r, T*_r) + FOM/100) / MAI_CO2_r  [EUR/tCO2]
        # use_discount_rate=True  → r from costs[fill_values][discount rate] (e.g. 0.07)
        # use_discount_rate=False → r=0  (annuity(T*,0)=1/T*, no time-value penalty)
        # Both branches use the same formula; setting r=0 recovers the no-discount case.
